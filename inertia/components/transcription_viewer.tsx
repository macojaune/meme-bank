...
export function TranscriptionViewer({ videoId, isOwner }: TranscriptionViewerProps) {
  ...
  const loadTranscription = async () => {
    try {
      setLoading(true)
      const response = await axios.get(`/api/v1/videos/${videoId}/transcription`)
  ...
  const submitCorrection = async () => {
    try {
      await axios.post(`/api/v1/videos/${videoId}/transcription/correct`, {
        text: editedText,
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-xs px-2 py-1 rounded ${
              transcription.status === 'auto_generated'
                ? 'bg-yellow-100'
        ...
        {isEditing ? (
          ...
          <>
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <button onClick={() => setIsEditing(true)} className="btn-neo text-xs px-3 py-1">
                ✏️ Corriger (+10 pts)
              </button>
            </div>
          </>
